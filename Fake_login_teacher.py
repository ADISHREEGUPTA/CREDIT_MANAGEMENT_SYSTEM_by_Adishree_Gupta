...
def signin():
    if userentry.get() == '' or passentry.get() == '':
        ...
        try:
            ...
            cur = con.cursor()
            cur.execute('select * from teacher where username=%s and password=%s', (userentry.get(), passentry.get()))
            row = cur.fetchone()
            if row == None:
...
def forgot_password():
    ...
    def authentication():
        ...
        if user_entry.get()==" " or DOB_entry.get()==" " or newpass_entry==" ":
            ...
            con = sq.connect(host='localhost',user='root',password='adishree', database='credit_management_system')
            cur = con.cursor()
            query='select * from teacher where username=%s and date_of_birth=%s'
            cur.execute(query,(user_entry.get(),DOB_entry.get()))
            row=cur.fetchone()
            if row==None:
                message.showerror('Error','Incorrect username and date of birth',parent=window)
            else:
                query='update teacher set password=%s where username=%s and date_of_birth=%s'
...
label = Label(root, text="TEACHER LOGIN", font=('Algerian', 25),bg="white")
