...
def signin():
    if userentry.get() == '' or passentry.get() == '':
        ...
        try:
            ...
            cur = con.cursor()
            cur.execute('select * from admin where username=%s and password=%s', (userentry.get(), passentry.get()))
            row = cur.fetchone()
            if row == None:
...
label = Label(root, text="ADMIN LOGIN", font=('Algerian', 25),bg="white")
